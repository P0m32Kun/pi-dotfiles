...
name: dev-workflow
description: |
  Full feature development workflow: align → design → review → parallel implement → review → simplify → test → archive → push.
  For large features, cross-module changes, frontend-backend coordination.
...
加载 align skill，评估 {task} 的清晰度：

- 如果任务已明确（有接口、行为、边界定义）→ 快速确认，标记 fast-track
- 如果任务模糊 → 执行 grilling 会话：
  - 读取 CONTEXT.md / CONTEXT-MAP.md 和相关 ADR
  - 逐条追问用户（每次一个问题）
  - 更新 CONTEXT.md 和 ADR
  - 产出对齐报告（含接口定义、行为约束、验收标准）
- 用户可随时说 "跳过对齐" 或 "直接开始" 以 fast-track

## Step 1: explorer

output: design-plan.md
reads: alignment-report.md

基于对齐报告，加载 recon-and-plan skill 分析 {task}，输出设计+计划合二为一的文档。包含：

- 需求分析与边界定义（复用对齐结果，不重复追问）
- 架构方案（至少 2 个备选 + 推荐）
- 垂直切片任务拆分（每个切片 ≤ 5 个文件）
- 依赖图与执行顺序
- 风险评估与缓解策略
