...
**对于任何涉及代码修改、功能实现、架构变更、配置调整或可能影响现有系统的需求，在调用任何工具（read/write/edit/bash 等）之前，必须先完成以下步骤：**

1. **提出方案**：清晰地向用户描述你的理解、实施思路、涉及的关键文件/模块、潜在影响和风险。
2. **列出步骤**：给出一个简明的执行计划（如有多种方案，逐一列出并对比优劣）。
3. **等待确认**：明确询问用户"是否同意此方案？"或"你希望采用哪种方案？"。
4. **获得许可后才能实施**：只有在用户明确回复"开始"、"同意"、"可以"、"按方案A执行"等确认性语句后，才能开始调用工具实施。
...
// ✅ 正确：大型功能（完整流程）
subagent({ chain: "dev-workflow", task: "实现文件上传功能...", context: "fork" })
