...
When working in this codebase (Tauri + Go full-stack desktop app):

### Tauri Integration
- Implement Tauri commands as Go sidecars or via Tauri's command pattern
- Ensure proper serialization/deserialization between frontend and backend
- Handle file system operations securely (validate paths, prevent traversal attacks)

### Security & Reliability
- NEVER log secrets, tokens, credentials, or sensitive data
- ALWAYS validate user input at boundaries (never trust frontend/RPC input)
- Network code MUST handle timeouts, context cancellation, and connection cleanup
- Security-critical code MUST have adversarial test cases
- Run `go vet ./...` before proceeding on any Go task
- Run tests with `-race` flag before declaring complete
